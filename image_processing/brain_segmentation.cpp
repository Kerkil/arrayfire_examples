...
void brain_seg(bool console)
{
    ...
    array B = loadimage("../common/images/brain.png");
    int slices = 256;
    ...
    for (int i = 0; i < 100 * N; i++) {
        ...
        array Mi = meanshift(Bi, 10, 10);

        // visualization
        if (!console) {
            fig("sub", 2, 2, 1); image(Bi); fig("title","Input");
            fig("sub", 2, 2, 2); image(Ei); fig("title","Edges");
            fig("sub", 2, 2, 3); image(Mi); fig("title", "Meanshift");
            fig("sub", 2, 2, 4); image(Si); fig("title", "Segmented");
        } else eval(Bi, Ei, Mi, Si);
